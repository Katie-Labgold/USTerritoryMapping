...
For this vignette, we'll be using one of the provided datasets `census.uninsured19`. `census.uninsured19.co` provides an example of a dataset with complete data for all US state and territory counties and county-equivalent units. It is already in the proper format for the provided package functions.
...
Note that the default option for county geometry data year (`county_data_year`) is `all.geo.co_2020` which provides the 2020 county geometry. `all.geo.co_2010` can be used to map using the 2010 county geometry file.
```{r, warnings = FALSE, message = FALSE, eval = FALSE}
map1_categorical_county(data = census.uninsured19.co, 
                        ...
                        state_color = "black", 
                        state_linewidth = 1,
                        save.filepath = "saved-maps/map1-co-uninsure.png")
```
