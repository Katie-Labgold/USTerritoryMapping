#---------------------------------------#
# Testing data merges w/ and w/o complete territory data 
#
# K Labgold
# 2023-06-01
#---------------------------------------#
...
states <- get_acs(year = 2020, output = "wide", variables = vars.acs, 
                        geography = "state", sumfile = "acs5") %>% # includes PR
                    left_join(fips, by = c("NAME" = "state_name")) %>%
                    mutate(
